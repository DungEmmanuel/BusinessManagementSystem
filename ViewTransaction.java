...
public class ViewTransaction extends JPanel implements ActionListener
{
	   ...
	   int productAmount, productQuantity, branchID;
	   int productTotal=0;
    ...
    public void viewTrans(String branchName)
    {
			...
			DBConnection konnect=new DBConnection();  // create a connection to the DB.orders
			...
			try 
			{
				while(result.next())               // loop through the returned result.
				{
					...
					Object[] rows={productName, productQuantity, productAmount,paymentStatus, "", ""};
					//model=(DefaultTableModel)itemsCart.getModel();
					model.addRow(rows);
